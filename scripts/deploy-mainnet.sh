#!/usr/bin/env bash
# =============================================================================
# iPredict — Mainnet Deploy Script
# =============================================================================
# Usage:
#   bash scripts/deploy-mainnet.sh
#
# Prerequisites:
#   1. The ipredict-deployer key must already be in your stellar keystore
...
echo -e "  RPC:       $RPC_URL"
echo -e "  Deployer:  $(stellar keys address ipredict-deployer 2>/dev/null || echo 'NOT FOUND')"
echo ""
...
DEPLOYER=$(stellar keys address ipredict-deployer 2>&1) || \
  error "ipredict-deployer key not found in keystore. Run testnet deploy first or add key manually."
...
for WASM in ipredict_token leaderboard referral_registry prediction_market; do
...
info "1/5 ipredict_token..."
B=$(get_bal)
stellar contract invoke --network "$NETWORK" --source-account ipredict-deployer \
  --id "$TOKEN_ID" -- initialize \
  --admin "$DEPLOYER" --name "IPREDICT" --symbol "IPRED" --decimals 7 2>&1 | \
  grep -v "^ℹ️\|^🌎\|^🔗"
