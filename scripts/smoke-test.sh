...
#   - ipredict-deployer identity in stellar keystore
...
echo -e "${BOLD}iPredict Testnet Smoke Test${NC}"
...
MARKET_RESULT=$(invoke --id "$MARKET_ID" \
  ...
  --image_url "https://ipredict.test/smoke.png" \
...
MARKET2=$(invoke --id "$MARKET_ID" \
  ...
  --image_url "https://ipredict.test/cancel.png" \
...
pass "IPREDICT token balance: $ALICE_TOKEN_BAL (expected 11_0000000 = 1 welcome + 10 win)"
...
pass "Bob IPREDICT tokens: $BOB_TOKENS (expected 2_0000000)"
