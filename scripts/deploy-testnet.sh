#!/usr/bin/env bash
# =============================================================================
# iPredict — Testnet Deploy Script
# =============================================================================
# Usage:
...
# We import the key into a temp stellar identity named "ipredict-deployer"
# so subsequent commands use --source-account ipredict-deployer (reads keystore, not env).
step "Importing deployer identity"

# Remove any stale identity first
stellar keys rm ipredict-deployer 2>/dev/null || true

# Add the secret key to the local stellar keystore (file at ~/.config/stellar/identity/)
echo "$DEPLOYER_SECRET" | stellar keys add ipredict-deployer --secret-key 2>&1

DEPLOYER_PUBLIC=$(stellar keys show ipredict-deployer)
...
deploy_contract() {
  ...
  WASM_HASH=$(stellar contract upload \
    --network "$NETWORK" \
    --source-account ipredict-deployer \
    --wasm "$WASM" \
...
TOKEN_ID=$(deploy_contract "ipredict_token"     "$WASM_DIR/ipredict_token.wasm")
...
# 1. Token contract
info "Initializing ipredict_token..."
stellar contract invoke \
  ...
  --symbol "IPRED" \
  --decimals 7
success "ipredict_token initialized"
...
if [[ -n "${SPONSOR_SECRET:-}" ]]; then
  ...
  SPONSOR_PUBLIC=$(stellar keys show ipredict-deployer 2>/dev/null || \
    stellar keys generate ipredict-sponsor --secret-key <<< "$SPONSOR_SECRET" && \
    stellar keys show ipredict-sponsor)
...
echo -e "${BOLD}${GREEN}║              iPredict Testnet Deploy Complete!               ║${NC}"
echo -e "${BOLD}${GREEN}╚══════════════════════════════════════════════════════════════╝${NC}"
echo ""
...
# If you want to remove it: stellar keys rm ipredict-deployer
info "Identity 'ipredict-deployer' saved to stellar keystore for future CLI calls."
info "To remove it later: stellar keys rm ipredict-deployer"
